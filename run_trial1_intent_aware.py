#!/usr/bin/env python3
"""
Run intent-aware behavioral simulation for Trial1 (AI tool for indie founders/SaaS teams).
...
def main():
    print("\n" + "=" * 80)
    print("🧠 INTENT-AWARE BEHAVIORAL SIMULATION: Trial1 (AI Tool)")
    print("=" * 80)
    print(f"   Personas: 1000")
    print(f"   Product Steps: {len(TRIAL1_STEPS)}")
    print(f"   Target Users: Indie Founders & Small SaaS Teams")
    ...
    # Use fixed global intent for Trial1
    # All users are indie founders/small SaaS teams wanting to try the AI tool
    print("\n🎯 Using fixed global intent for Trial1...")
    print(f"   Intent: {TRIAL1_GLOBAL_INTENT.intent_id}")
    print(f"   Description: {TRIAL1_GLOBAL_INTENT.description}")
    print(f"   Primary Goal: {TRIAL1_GLOBAL_INTENT.primary_goal}")
    print(f"   User Type: Indie Founders & Small SaaS Teams")
    ...
    result_df = run_intent_aware_simulation(
        df,
        product_steps=TRIAL1_STEPS,
        fixed_intent=TRIAL1_GLOBAL_INTENT,  # Fixed intent for all users
        verbose=True,
        seed=42
    )
    ...
    analysis = generate_intent_analysis(result_df, TRIAL1_STEPS)
    
    # Export artifacts
    print("\n💾 Exporting intent artifacts...")
    artifacts = export_intent_artifacts(result_df, TRIAL1_STEPS, output_dir=".")
    ...
    summary = {
        ...
        'completion_rate_ci_lower': float(ci_lower),
        'completion_rate_ci_upper': float(ci_upper),
        'completion_rate_std_error': float(std_error),
        'completion_rate_range': ci_results['completion_range'],
        'confidence_level': 0.95,
        'total_intent_mismatches': int(total_mismatches),
        'fixed_intent': TRIAL1_GLOBAL_INTENT.to_dict(),
    ...
    print(f"   Standard Error: {std_error:.1%}")
