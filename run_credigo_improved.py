#!/usr/bin/env python3
"""
Run improved behavioral simulation for Credigo with 1000 personas.
"""
...
def run_credigo_simulation_improved():
    """Run improved behavioral simulation for Credigo."""
    print("\n" + "=" * 80)
    print("🚀 CREDIGO IMPROVED BEHAVIORAL SIMULATION")
    print("=" * 80)
    print(f"   Personas: 1000")
    print(f"   Product: Credigo.club")
    ...
    try:
        ...
        # Build funnel table
        print(f"\n   Step-by-Step Funnel:")
        print(f"   {'Step':<30} {'Entered':<12} {'Exited':<12} {'Drop Rate':<12} {'Cumulative Loss':<15}")
        print(f"   {'-'*30} {'-'*12} {'-'*12} {'-'*12} {'-'*15}")
        
        cumulative_entered = total_trajectories
        cumulative_loss = 0
        
        for step_name in CREDIGO_11_STEPS.keys():
            ...
            print(f"   {step_name:<30} {entered:<12,} {exited:<12,} {drop_rate:<11.1f}% {cumulative_loss_pct:<14.1f}%")
        
        # Completed
        completed_count = step_completions.get('Completed', 0)
        print(f"   {'Completed':<30} {total_trajectories:<12,} {completed_count:<12,} {'N/A':<12} {'N/A':<15}")
...
if __name__ == "__main__":
    result_df, summary = run_credigo_simulation_improved()
